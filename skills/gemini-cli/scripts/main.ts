...
Options:
  ...
  -m, --model <id>          gemini-3.5-pro | gemini-3.5-pro-thinking | gemini-3.5-flash | gemini-3.1-pro-preview | gemini-3.1-flash-lite (default: gemini-3.5-pro)
  --json                    Output JSON
...
function resolveModel(id: string): Model {
  const k = id.trim();
  if (k === 'gemini-3.5-flash') return Model.G_3_5_FLASH;
  if (k === 'gemini-3.5-pro') return Model.G_3_5_PRO;
  if (k === 'gemini-3.5-pro-thinking') return Model.G_3_5_PRO_THINKING;
  if (k === 'gemini-3.1-pro-preview') return Model.G_3_1_PRO_PREVIEW;
  if (k === 'gemini-3.1-flash-lite') return Model.G_3_1_FLASH_LITE;
